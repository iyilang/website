 ...
 * `scripts/releases.mjs` recorded them, and the archives it publishes, read
 * out of `install.sh` and `install.ps1` as `scripts/install-targets.mjs`
 * recorded them. A version number and a target name are facts about the tree
 * like any other figure here, so neither is typed into a page: one generator
 * refuses to write a record whose changelog states no release, the other
 * refuses one whose installers and README disagree about what a release
 * ships.
 *
...
/**
 * An installer's own template, with its placeholders filled in. Both
 * spellings, because the two installers are two languages: `install.sh`
 * writes `$version` and `$target`, `install.ps1` writes `$Version`. Each
 * string is carried across as its script wrote it, and this is the only place
 * that knows they mean the same thing.
 */
...
/**
 * The Windows zip, which `install.ps1` fetches and `install.sh` refuses.
 * 0.14.0 is the first release to publish it. It is not one of the tarballs
 * above - another archive, another installer, one target - so it is offered
 * on its own rather than folded into a list a reader would then have to read
 * a file extension out of.
 */
export function windowsAsset(release: Release = latestRelease()): ReleaseAsset {
  const { target, machine } = install.windows;
  const file = fill(install.windows.asset, release.version);
  return {
    target,
    machine,
    file,
    url: `${fill(install.windows.download, release.version)}/${file}`,
  };
}
